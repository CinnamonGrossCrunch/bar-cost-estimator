...
// Pricing constants for GitHub Internal only
const PRICING = {
  fullBar: { perPerson: 8, minimum: 800 },
  beerWine: { perPerson: 6, minimum: 600 }
};

type ServiceType = 'fullBar' | 'beerWine';

// GitHub Internal configuration
const GITHUB_INTERNAL_CONFIG = {
...
export default function GitHubInternalWidget() {
  ...
  // No company name required for GitHub Internal
  const isFormValid = attendees > 0;
  ...
  }, [attendees, isFormValid, serviceType]);
  ...
  const handleGenerateEstimate = async () => {
    console.log('Generate estimate clicked', { isFormValid, attendees });
  ...
  }, [serviceType, attendees, duration]);
  ...
  const config = GITHUB_INTERNAL_CONFIG;
