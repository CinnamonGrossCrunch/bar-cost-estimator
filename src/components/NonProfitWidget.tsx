...
// Pricing constants for Non-Profit only
const PRICING = {
  fullBar: { perPerson: 8.5, minimum: 800 },
  beerWine: { perPerson: 6.5, minimum: 600 }
};

type ServiceType = 'fullBar' | 'beerWine';

// Non-Profit configuration
const NON_PROFIT_CONFIG = {
  label: 'Non-Profit',
...
export default function NonProfitWidget() {
  ...
  // Organization name is required for Non-Profit
  const isFormValid = attendees > 0 && organizationName.trim() !== '';
  ...
  }, [attendees, organizationName, isFormValid, serviceType]);
  ...
  const handleGenerateEstimate = async () => {
    console.log('Generate estimate clicked', { isFormValid, attendees, organizationName });
  ...
  }, [serviceType, attendees, duration, organizationName]);
  ...
  const config = NON_PROFIT_CONFIG;
  ...
  return (
    <div className={`bg-black rounded-b-12xl p-12 relative overflow-hidden w-full h-auto`}>
      ...
      {/* Organization Name - Required for Non-Profit */}
      <div className="mb-2 backdrop-blur-md rounded-xl p-4 relative overflow-hidden border border-white/10" style={{
        background: 'linear-gradient(to bottom, rgba(255,255,255,0.08), rgba(255,255,255,0.02))'
      }}>
        <label htmlFor="organizationName" className="block text-sm font-semibold text-gray-200 mb-2">
          Organization Name *
        </label>
        <input
          id="organizationName"
